...
pub fn search(categories: &Categories) -> String {
    ...
    format!(
      ...
      <form method="dialog" id="form" onSubmit="doSearch()">
        ...
        categories
            .iter()
            .flat_map(|category| category.genera.iter().flat_map(|genera| genera
                .species
                .iter()
                .map(|species| SearchInfo{category:category.title.clone(),species:format!("{} {}", genera.title, species.title)})))
            .collect::<Vec<SearchInfo>>()
            .into_iter()
            .fold("".to_string(), |a, info| format!(
                "{}\n<option>{}</option>",a,info.species,
            ))
    )
}
...
pub fn data(categories:&Categories) -> String {
        categories
            ...
            .flat_map(|category| category.genera.iter().flat_map(|genera| genera
                .species
                .iter()
                .map(|species| SearchInfo{category:category.label.clone(),species:format!("{} {}", genera.title, species.title)})))
            .collect::<Vec<SearchInfo>>()
            .into_iter()
            .fold("".to_string(), |a, info| format!(
                r#"{}"{}":"{}","#,a,info.species,info.category,
            ))
}
